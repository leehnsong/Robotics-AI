...
사용자가 **"빨간 블록을 파란 접시에 올려"** 같은 자연어 명령을 입력하면, 카메라로 장면을 인식하고 AI가 행동 계획을 세워 로봇 팔이 자율적으로 작업을 수행합니다.
...
| 번호 | 담당자 | 패키지 | 역할 | 브랜치 |
|------|--------|--------|------|--------|
| P1 | 이현송 | `vla_perception` | 카메라 + Gemini로 물체 3D 좌표 산출 | `feat/perception` |
| P2 | 김은수 | `vla_planner` | 자연어 명령 → 행동 시퀀스 계획 | `feat/planner` |
| P3 | 최유리 | `vla_control` + `arm_bringup` | MoveIt2로 팔 실제 실행, 그리퍼 제어 | `feat/control` |
| P4 | 신민서 | `vla_orchestrator` | 전체 흐름 조율 + 안전 관리 | `feat/orchestrator` |
| P5 | 정민기, 김예서 | `vla_interfaces` + `vla_bringup` | 인터페이스 정의 + 통합 런치 | `feat/bringup` |

> **협업 규칙**: 각자 자기 패키지 폴더만 수정 (merge 충돌 방지). `vla_interfaces`, `vla_bringup`은 P5 전담 / `arm_bringup`은 P3 전담.
